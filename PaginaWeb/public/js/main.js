...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        ...
        if (toggle) {
            ...
            var content = parent.querySelector('.project-full');
            if (content) {
                var isOpen = content.style.display === 'block';
                content.style.display = isOpen ? 'none' : 'block';
                toggle.textContent = isOpen ? toggle.getAttribute('data-text-original') : 'Ocultar';
            }
        }
    });
            ...
            try {
                ...
                if (response.success && response.data.length > 0) {
                    ...
                    response.data.forEach(function(item, index) {
                        html += '<article class="opinion-card' + (index === 0 ? ' active' : '') + '">';
...
function loadCalendarMonth() {
    ...
    html += '<button class="calendar-nav" data-calendar-prev aria-label="Mes anterior">&#9664;</button>';
    html += '<div class="calendar-selectors">';
    html += '<select class="calendar-select calendar-month-select" data-calendar-month aria-label="Mes">' + monthOptions + '</select>';
    html += '<select class="calendar-select calendar-year-select" data-calendar-year aria-label="Año">' + yearOptions + '</select>';
    html += '</div>';
    html += '<button class="calendar-nav" data-calendar-next aria-label="Mes siguiente">&#9654;</button>';
    html += '</div>';
    ...
    if (prevBtn) {
        prevBtn.addEventListener('click', calendarPrev);
    }
    var monthSelect = container.querySelector('[data-calendar-month]');
    if (monthSelect) {
        monthSelect.addEventListener('change', function() {
            calendarGoToMonth(parseInt(this.value));
        });
    }
    var yearSelect = container.querySelector('[data-calendar-year]');
    if (yearSelect) {
        yearSelect.addEventListener('change', function() {
            calendarGoToYear(parseInt(this.value));
        });
    }
    var nextBtn = container.querySelector('[data-calendar-next]');
    if (nextBtn) {
        nextBtn.addEventListener('click', calendarNext);
    }
    ...
    apiGet(url, function(response) {
        if (response.data.length > 0) {
            ...
            response.data.forEach(function(group) {
                var titulo = escapeHtml(group.titulo || '');
                ...
                if (imagenes.length === 1) {
                    ...
                    html += '<div class="gallery-card gallery-carousel-card" data-images=\'' + escapeHtml(JSON.stringify(imagenes)) + '\' data-title="' + titulo + '">';
            ...
            container.innerHTML = html;
            initCardCarousels(container);
            if (!container.dataset.galleryListener) {
                container.dataset.galleryListener = '1';
                container.addEventListener('click', function(e) {
                    var carouselCard = e.target.closest('.gallery-carousel-card');
                    if (carouselCard) {
                        var images = [];
                        try { images = JSON.parse(carouselCard.dataset.images); } catch(ex) {}
                        var title = carouselCard.dataset.title || '';
                        openGalleryLightbox(images, title);
                        return;
                    }
                    var singleCard = e.target.closest('.gallery-card[data-src]');
                    if (singleCard) openLightbox(singleCard.dataset.src, singleCard.dataset.title);
                });
            }
        } else {
...
function openLightbox(src, alt) {
    ...
    lightboxImages = [];
    galleryImages.forEach(function(el) { lightboxImages.push(el.src); });
...
function openGalleryLightbox(images, title) {
    ...
    lb.className = 'gallery-lb-overlay';
    ...
    var singleNav = images.length <= 1 ? ' style="display:none"' : '';

    lb.innerHTML =
        '<button class="gallery-lb-close" aria-label="Cerrar">&times;</button>' +
        '<div class="gallery-lb-body">' +
            '<button class="gallery-lb-prev gallery-lb-nav"' + singleNav + ' aria-label="Anterior">&#10094;</button>' +
            '<div class="gallery-lb-carousel">' +
                '<div class="gallery-lb-track">' + slidesHtml + '</div>' +
            '</div>' +
    ...
    var currentIdx = 0;
    var track = lb.querySelector('.gallery-lb-track');
    var allDots = lb.querySelectorAll('.gallery-lb-dot');
    var autoTimer = null;
    ...
    function goTo(idx) {
        ...
        allDots.forEach(function(d, i) { d.classList.toggle('active', i === currentIdx); });
    }
    ...
    lb.querySelector('.gallery-lb-prev').addEventListener('click', function(e) {
        e.stopPropagation();
        goTo(currentIdx - 1);
        startAuto();
    });

    lb.querySelector('.gallery-lb-next').addEventListener('click', function(e) {
        e.stopPropagation();
        goTo(currentIdx + 1);
        startAuto();
    });
    ...
    lb.querySelector('.gallery-lb-close').addEventListener('click', function(e) {
        e.stopPropagation();
        closeGalleryLightbox();
    });

    lb.addEventListener('click', function(e) {
        var inCarousel = e.target.closest('.gallery-lb-carousel');
        var inNav = e.target.closest('.gallery-lb-nav');
        var inDots = e.target.closest('.gallery-lb-dots');
        if (!inCarousel && !inNav && !inDots) {
            closeGalleryLightbox();
        }
    });

    var touchSX = 0;
    lb.querySelector('.gallery-lb-carousel').addEventListener('touchstart', function(e) {
        touchSX = e.changedTouches[0].screenX;
        stopAuto();
    }, { passive: true });
    lb.querySelector('.gallery-lb-carousel').addEventListener('touchend', function(e) {
